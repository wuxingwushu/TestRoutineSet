...
struct Arm_DutyCycle_Angle
{
    ...
    unsigned int GetPwm(float angle) {
        unsigned int Pwm = VerticalF - VerticalT;
        Pwm *= angle / M_PI;
...
struct Arm_Result
{
    float AneleArm;//机械臂爪子旋转
    float AngleBCD;//BC与CD的夹角
    float AngleABC;//AB与BC的夹角
    float AngleOAB;//AB与底盘平面的夹角
    float AngleTerritory;//底盘旋转
    bool Success;//是否成功
};
