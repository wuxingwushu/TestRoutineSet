...
int main()
{
    ...
    Box mbox = Box(0.0f, 0.0f, 1.0f, 1.0f);
    // Add nodes to quadtree
    auto mquadtree = Quadtree<Node*, decltype(getBox)>(mbox, getBox);
    Node addd;
    addd.box.left = 0.5f;
    addd.box.top = 0.5f;
    addd.box.width = 0.2f;
    addd.box.height = 0.2f;
    addd.id = 12;
    mquadtree.add(&addd);
    ...
    Box cha = Box(0.69f, 0.69f, 0.8f, 0.8f);

    std::vector<Node*> kao = mquadtree.query(cha);
    for (size_t i = 0; i < kao.size(); i++)
    {
        std::cout << kao[i]->id << '\n';
    }
