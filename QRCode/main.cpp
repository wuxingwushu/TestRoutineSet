...
void QR() {
	qrcodegen::QrCode Qr = qrcodegen::QrCode::encodeText("314159265358979323846264338327950288419716939937510", qrcodegen::QrCode::Ecc::LOW);
	int border = 2;
	for (int y = -border; y < Qr.getSize() + border; y++) {
		for (int x = -border; x < Qr.getSize() + border; x++) {
			if (!Qr.getModule(x, y)) {
				GetPix(x + border, y + border);
			}
		}
	}
}
